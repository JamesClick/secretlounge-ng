...
class Sender(Receiver):  # flawless class hierachy I know...
    ...
    def reply(m, msid, who, except_who, reply_to):
        logging.debug(
            "reply(m.type=%s, msid=%r, reply_to=%r)",
            rp.types.reverse[m.type],
            msid,
            reply_to,
        )
...
def toggle_karma(user):
    with db.modifyUser(id=user.id) as user:
        ...
        new = user.hideKarma
    return rp.Reply(
        rp.types.BOOLEAN_CONFIG, description="Karma notifications", enabled=not new
    )
...
def uncooldown_user(user, oid2=None, username2=None):
    ...
    logging.info(
        "%s removed cooldown from %s (was until %s)",
        user,
        user2,
        format_datetime(was_until),
    )
    return rp.Reply(rp.types.SUCCESS)
...
def send_signed_user_message(user, msg_score, text, reply_msid=None, tripcode=False):
    ...
    if not tripcode:
        ...
        if last_used and (datetime.now() - last_used) < timedelta(
            seconds=SIGN_INTERVAL_SECONDS
        ):
