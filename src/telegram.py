...
def calc_spam_score(ev):
    ...
    if ev.content_type == "sticker":
        ...
        return s
    s += (
        len(ev.text) * SCORE_TEXT_CHARACTER + ev.text.count("\n") * SCORE_TEXT_LINEBREAK
    )
    return s
...
def resend_message(chat_id, ev, reply_to=None):
    ...
    if ev.content_type == "text":
        ...
        return bot.send_document(
            chat_id, ev.document.file_id, caption=ev.caption, **kwargs
        )
...
def send_to_single_inner(chat_id, ev, **kwargs):
    if isinstance(ev, rp.Reply):
        ...
        return bot.send_message(
            chat_id, rp.formatForTelegram(ev), parse_mode="HTML", **kwargs2
        )
    else:
...
def relay(ev):
    ...
    if (
        not allow_documents
        and ev.content_type == "document"
        and ev.document.mime_type not in ("image/gif", "video/mp4")
    ):
        return
    ...
    if ev.reply_to_message is not None:
        reply_msid = ch.lookupMapping(
            ev.from_user.id, data=ev.reply_to_message.message_id
        )
        if reply_msid is None:
...
def cmd_tsign(ev, arg):
    ...
    msid = core.send_signed_user_message(
        c_user, calc_spam_score(ev), arg, reply_msid, tripcode=True
    )
    if isinstance(msid, rp.Reply):
        return send_answer(ev, msid, True)
