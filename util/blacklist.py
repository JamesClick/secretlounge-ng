...
def unban_user(db, id):
    c = db.execute(
        "SELECT realname, left FROM users WHERE id = ? AND rank = ?", (id, -10)
    )
    row = c.fetchone()
    if row is None:
        return 0
...
def sync(d):
    ...
    while True:
        ...
        for name, db in d.items():
            c = db.execute(
                "SELECT id, blacklistReason FROM users WHERE rank = ? AND left >= ?",
                (-10, last_update),
            )
            for row in c:
                reason = row[1] or ""
        ...
        for id, reason, from_name in l:
            ...
            if stat1 + stat2 > 0:
                logging.info(
                    "Transferred ban of user id %d orignated from %s (%d-%d)",
                    id,
                    from_name,
                    stat1,
                    stat2,
